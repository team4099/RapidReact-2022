...
object LedIOCandle : LedIO {
  ...
  override fun setMode(mode: BlinkinLedDriver.BlinkinLedMode) {
    lastLedState = mode
    when (mode) {
      BlinkinLEDState.IDLE.blinkinMode -> setCandleState(CandleState.IDLE)
      BlinkinLEDState.AUTO.blinkinMode -> setCandleState(CandleState.AUTO)
      BlinkinLEDState.STANDING_ZERO.blinkinMode -> setCandleState(CandleState.STANDING_ZERO)
      BlinkinLEDState.STANDING_ONE.blinkinMode -> setCandleState(CandleState.STANDING_ONE)
      BlinkinLEDState.STANDING_TWO.blinkinMode -> setCandleState(CandleState.STANDING_TWO)
      BlinkinLEDState.INTAKING.blinkinMode -> setCandleState(CandleState.INTAKING)
      BlinkinLEDState.OUTTAKING.blinkinMode -> setCandleState(CandleState.OUTTAKING)
      BlinkinLEDState.CLIMBER_READY.blinkinMode -> setCandleState(CandleState.CLIMBER_READY)
      BlinkinLEDState.CLIMBING.blinkinMode -> setCandleState(CandleState.CLIMBING)
      BlinkinLEDState.DISALLOW_CLIMB.blinkinMode -> setCandleState(CandleState.DISALLOW_CLIMB)
